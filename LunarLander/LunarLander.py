...
ship_mass = 5000.0  # 啥玩意？
fuel = 5000.0
velocity = -100.0  # 下降/上升速度
gravity = 10
height = 2000
thrust = 0  # 推力
delta_v = 0  # 速度变化量
...
class ThrottleClass(pygame.sprite.Sprite):
    def __init__(self, location=[0,0]):
        pygame.sprite.Sprite.__init__(self)  # 推进器类（self）继承动画精灵
        image_surface = pygame.surface.Surface([30,10])  # 生成一个推进器
        image_surface.fill([128,128,128])  # 颜色填充
        self.surface = image_surface.convert()  # 把 Surface 对象进行转化，提升 blit 时的效率
...
def calculate_velocity():
    ...
    delta_t = 1/fps  # 时间变化量
    thrust = (500 - myThrottle.rect.centery) * 5.0  # 将推进器精灵的 y 位置转换为推力
    fuel -= thrust / (10 * fps)  # 根据推力减少燃料
    if fuel < 0: fuel = 0.0
    if fuel < 0.1: thrust = 0.0
    delta_v = delta_t * (-gravity + 200 * thrust / (ship_mass + fuel))  # 速度变化量
    velocity = velocity + delta_v  # 计算速度
    delta_h = velocity * delta_t  # 高度变化量
...
while running:
    ...
    if height > 0.01:
        ...
        pygame.draw.rect(screen,[0,255,0],
                         [84,448-fuelbar,18,fuelbar],0)  # 更新绿色燃油量
        pygame.draw.rect(screen,[255,0,0],
                         [25,300,10,200],0)  # 画出红色推进器滑块
        screen.blit(moon,[0,500,400,100])  # 画月球
        pygame.draw.rect(screen,[60,60,60],
                         [220,535,70,5],0)  # 画降落点
        screen.blit(myThrottle.surface, myThrottle.rect)  # 画推力操作杆
        display_flames()
